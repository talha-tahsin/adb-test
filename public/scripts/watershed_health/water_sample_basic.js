...
$(document).on('click', '#btn_store', function (event) {
    ...
    else if(sample_id == '' || sample_id == undefined) {
        var errorMsg = 'Please enter sample id..';
        $('#myModal').modal({backdrop : 'static', keyboard : false});
        $('#error_msg').html('<span style="color: red">ERROR!! <p>'+ errorMsg +'</p></span>');
    }
...
$(document).on('click', '#btn_data_entry', function(){
    ...
    var waterShed_id = $(this).closest('tr').find('td:eq(2)').text();
    var waterShed_name = $(this).closest('tr').find('td:eq(3)').text();
    var sample_id = $(this).closest('tr').find('td:eq(4)').text();
    ...
    console.log(waterShed_id, waterShed_name, sample_id, para_id, para_name);

    // window.location.href = '/gps-point-of-para-boundary';
    window.open('/water-quality-entry', '_blank');

    if(sample_id == '' || sample_id == null || sample_id == undefined){
    ...
    else 
    {
        ...
        $.ajax({
            url: "/store_sample_id_random_entry",
            type: "POST",
